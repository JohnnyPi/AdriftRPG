...
pub fn texture_recipe_from_yaml_value(
    value: &serde_yaml::Value,
) -> Result<TextureRecipe, TextureGenerationError> {
    ...
    match variant {
        "Rock" => Ok(TextureRecipe::Rock(
            serde_yaml::from_value(config_value.clone())
                .map_err(|e| TextureGenerationError::InvalidConfig(format!("Rock config: {e}")))?,
        )),
        "Ground" => Ok(TextureRecipe::Ground(
            serde_yaml::from_value(config_value.clone()).map_err(|e| {
                TextureGenerationError::InvalidConfig(format!("Ground config: {e}"))
            })?,
        )),
        "Sand" => Ok(TextureRecipe::Sand(
            serde_yaml::from_value(config_value.clone())
                .map_err(|e| TextureGenerationError::InvalidConfig(format!("Sand config: {e}")))?,
        )),
...
impl TextureRecipe {
    ...
    pub fn fingerprint(&self) -> [u8; 32] {
        let json = serde_json::to_string(self)
            .unwrap_or_else(|err| format!("TextureRecipe::fingerprint serialize error: {err}"));
        *Hasher::new().update(json.as_bytes()).finalize().as_bytes()
    }
}

pub trait ProceduralTextureGenerator: Send + Sync + 'static {
    fn generate(&self, width: u32, height: u32)
    -> Result<GeneratedPbrMaps, TextureGenerationError>;
