...
pub struct WeatherState {
    ...
    /// When true, this world has a weather profile and drives animated cloud cover.
    pub animates_cloud_cover: bool,
}
...
fn init_weather_from_registry(
    ...
    let Ok(world) = registry.0.effective_world(Some(&world_id)) else {
        weather.animates_cloud_cover = false;
        return;
    };
    if let Some(profile) = registry.0.effective_weather(world) {
        weather.animates_cloud_cover = true;
...
fn animate_weather(
    ...
    if weather.animates_cloud_cover {
        let world_id = requested_world_id(&prefs);
